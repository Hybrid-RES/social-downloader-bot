...
Приватный Telegram-бот для домашнего сервера: отправляете ссылку с телефона через **Поделиться → Telegram**, а бот самостоятельно скачивает фото, видео, GIF или аудио на сервер.
...
- SQLite — постоянная очередь, история и защита от повторной постановки URL;
...
| Threads | `yt-dlp` | `gallery-dl` |
| LinkedIn | `yt-dlp` | `gallery-dl` |
| Другой сайт | `yt-dlp` | `gallery-dl` |

Для Twitter и Instagram `gallery-dl` может передавать видео внутреннему модулю `yt-dlp`. Threads и LinkedIn считаются экспериментальными: результат зависит от конкретной страницы и текущей поддержки загрузчиков.
...
```text
/status        статистика очереди и свободное место
...
SQLite хранит URL, статус, попытки, путь, размер и ошибку. После перезапуска задания в состояниях `downloading` и `postprocessing` возвращаются в очередь.
